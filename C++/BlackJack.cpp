...
int main(int argc, char *argv[]) {
    ...
    /*
    bet -> amount the user would like to bet
    playerCard -> player's dealt card
    dealerCard -> dealer's dealt card
    playerTotal -> total after player's dealt card's
    dealerTotal -> total after dealer's dealt card's
    iterator -> value to determine how many times a loop iterates
    money -> how much credit the player has
    choice -> user's selection from the menu
    pCardValue -> player's card face-value
    dCardValue -> dealer's card face-value
    hit -> take another card from the dealer (y/n)
    outcome -> whether the player won, lost, or pushed
    goAgain -> whether the player would like to choose another option from the menu or play again
    outfile -> file to be written to containing game history information
    */
